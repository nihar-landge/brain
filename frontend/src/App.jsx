...
import { Brain, LayoutDashboard, BookOpen, MessageCircle, Target, BarChart3, Settings, Users, Clock, FlaskConical, MoreHorizontal, Candy, ListChecks, CalendarDays, Heart, FileText, ChevronDown } from 'lucide-react'
...
// Grouped nav items (desktop dropdowns)
const navGroups = [
  {
    label: 'Productivity',
    icon: Clock,
...
export default function App() {
  ...
  useEffect(() => {
    const handleClick = (e) => {
      ...
      if (groupRef.current && !groupRef.current.contains(e.target)) setOpenGroup(null)
    }
      ...
      <div className="min-h-screen bg-white">
        ...
        <header className="sticky top-0 z-20 bg-white border-b border-gray-200 hidden sm:block">
          <div className="max-w-6xl mx-auto px-6 flex items-center justify-between h-14">
            ...
            {/* Center Nav */}
            <nav className="flex items-center gap-1" ref={groupRef}>
                ...
                return (
                  <div key={group.label} className="relative">
                    <button
                      ...
                      className={`nav-link flex items-center gap-1.5 ${isGroupActive ? 'nav-link-active' : ''}`}
                    >
                    ...
                    {openGroup === group.label && (
                      <div className="absolute top-full left-0 mt-1 w-48 bg-white border border-gray-200 rounded-lg shadow-lg py-1 z-30 animate-in">
                        {group.items.map(({ to, icon: Icon, label }) => (
                          <NavLink
                            key={to}
                            to={to}
                            className={({ isActive }) =>
                              `flex items-center gap-3 px-4 py-2.5 text-sm transition-colors ${isActive ? 'bg-gray-100 text-black font-medium' : 'text-gray-600 hover:bg-gray-50'}`
                            }
                          >
                            <Icon className="w-4 h-4" />
                            <span>{label}</span>
          ...
          <div className="flex items-center justify-around h-16">
            {mobileNavItems.map(({ to, icon: Icon, label }) => (
              <NavLink
                ...
                className={({ isActive }) =>
                  `flex flex-col items-center justify-center gap-0.5 py-2 px-3 min-w-[56px] transition-colors ${isActive ? 'text-black' : 'text-gray-400'
                  }`
                }
              >
            ...
            <div className="relative" ref={moreRef}>
              <button
                ...
                className={`flex flex-col items-center justify-center gap-0.5 py-2 px-3 min-w-[56px] transition-colors ${isOverflowActive || showMore ? 'text-black' : 'text-gray-400'
                  }`}
              >
                ...
                <div className="absolute bottom-full right-0 mb-2 w-48 bg-white border border-gray-200 rounded-lg shadow-lg py-1">
                  {mobileOverflowItems.map(({ to, icon: Icon, label }) => (
                    <NavLink
                      ...
                      className={({ isActive }) =>
                        `flex items-center gap-3 px-4 py-3 text-sm transition-colors ${isActive ? 'bg-gray-100 text-black font-medium' : 'text-gray-600'
                        }`
                      }
                    >
                      <Icon className="w-4 h-4" />
                  ...
                  <NavLink
                    to="/settings"
                    className={({ isActive }) =>
                      `flex items-center gap-3 px-4 py-3 text-sm transition-colors border-t border-gray-100 ${isActive ? 'bg-gray-100 text-black font-medium' : 'text-gray-600'
                      }`
                    }
                  >
                    <Settings className="w-4 h-4" />
