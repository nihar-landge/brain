...
async def predict_mood(req: MoodPredictionRequest, db: Session = Depends(get_db)):
    """
    Predict mood for a specific date.
    Uses adaptive ML with confidence scoring (Fix #5).
    """
    ml_service = MLService(db)
    return ml_service.predict_mood(user_id=1, target_date=req.date)


@router.post("/habit", response_model=dict)
async def predict_habit_success(
    req: HabitPredictionRequest, db: Session = Depends(get_db)
):
    """
...
async def predict_energy(days_ahead: int = 7, db: Session = Depends(get_db)):
    """Forecast energy levels for the next N days."""
    ml_service = MLService(db)
    return ml_service.get_energy_forecast(user_id=1, days_ahead=days_ahead)


@router.get("/status", response_model=dict)
async def get_prediction_status(db: Session = Depends(get_db)):
    ...
    return predictor.get_data_status(user_id=1)


@router.post("/retrain", response_model=dict)
async def retrain_models(model_name: str = "all", db: Session = Depends(get_db)):
    """Trigger model retraining."""
    ml_service = MLService(db)
    return ml_service.retrain_models(user_id=1, model_name=model_name)
...
async def get_model_performance(db: Session = Depends(get_db)):
    ...
    ml_service = MLService(db)
    return ml_service.get_model_performance(user_id=1)
